...
export default function LahanEdit({ lahan }: { lahan: Lahan }) {
    const { data, setData, put, processing, errors } = useForm({
        nama_blok: lahan.nama_blok,
        luas_ha: lahan.luas_ha.toString(),
    });
    ...
    return (
        <AppHeaderLayout>
            <Head title={`Edit Lahan: ${lahan.nama_blok}`} />
            <div className="flex h-full w-full flex-col gap-4 p-4 lg:p-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight">Edit Lahan</h1>
                        <p className="text-sm text-muted-foreground">Ubah informasi blok kebun sawit.</p>
                    </div>
                </div>
                    ...
                    <Card className="rounded-[20px] shadow-sm">
                        <form onSubmit={submit}>
                            <CardHeader>
                                ...
                                <CardDescription>Perbarui detail lahan di bawah ini.</CardDescription>
                            </CardHeader>
